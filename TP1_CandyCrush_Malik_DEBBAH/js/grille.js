...
class Grille {
  ...
  detecterMatch3Lignes(){
    ...
    for (let l = 0; l < this.nbLignes; l++) {
      this.selectionneeAlignementLigne(l);
    }
  ...
  detecterMatch3Colonnes(){
    ...
    for (let l = 0; l < this.nbLignes; l++) {
      this.selectionneeAlignementColonne(l);
    }
  ...
  selectionneeAlignementLigne(ligne) {
    // on va parcourir la ligne et voir si on des alignements
    let ligneGrille = this.tabCookies[ligne];
  ...
  selectionneeAlignementColonne(colonne) {
    // on veut afficher les cookies situées sur une colonne donnée
    for (let ligne = 0; ligne <= this.nbLignes - 3; ligne++) {
     // console.log("On va examiner la case " + ligne + " " + colonne);
      let cookie1 = this.tabCookies[ligne][colonne];
  ...
  chute(colonne) {
    // je test mes chute sur une colonne
    // je part du bas de la grille
    for (let ligne = 8; ligne >= 0; ligne--) {
      console.log("On va examiner la case " + ligne + " " + colonne);
      let cookie1 = this.tabCookies[ligne][colonne];
      console.log(cookie1.htmlImage.classList);

      //je test si une case contient une cookie vide et si elle n'est pas eu haut de la grille
      if (cookie1.htmlImage.classList == 'cookies cookie-cachee' && cookie1.ligne != 0) {


        // J'ai tenté d'inverser la position du cookie invisible et du cookie au dessus de celui invisible pour simuler une chute 
        console.log("bonjour");
        let cookie2 = this.tabCookies[ligne-1][colonne];
